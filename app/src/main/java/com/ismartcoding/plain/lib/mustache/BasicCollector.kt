...
import com.ismartcoding.plain.lib.mustache.Mustache.CustomContext
import com.ismartcoding.plain.lib.mustache.Mustache.VariableFetcher
...
abstract class BasicCollector : ICollector {
    ...
    override fun createFetcher(
        ...
        if (ctx is Mustache.CustomContext) return CUSTOM_FETCHER
    ...
    abstract class ArrayHelper : Mustache.VariableFetcher {
    ...
    companion object {
        ...
        protected val CUSTOM_FETCHER =
            object : Mustache.VariableFetcher {
                @Throws(Exception::class)
                override fun get(
                    ctx: Any,
                    name: String,
                ): Any {
                    val custom = ctx as Mustache.CustomContext
                    val `val` = custom[name]
