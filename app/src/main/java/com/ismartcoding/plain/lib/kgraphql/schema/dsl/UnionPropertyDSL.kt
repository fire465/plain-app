...
class UnionPropertyDSL<T : Any>(val name : String, block: UnionPropertyDSL<T>.() -> Unit) : LimitedAccessItemDSL<T>(), ResolverDSL.Target {
    ...
    fun resolver(function: suspend (T) -> Any?) = resolver(FunctionWrapper.Companion.on(function, true))

    fun <E> resolver(function: suspend (T, E) -> Any?) = resolver(FunctionWrapper.Companion.on(function, true))

    fun <E, W> resolver(function: suspend (T, E, W) -> Any?) = resolver(FunctionWrapper.Companion.on(function, true))

    fun <E, W, Q> resolver(function: suspend (T, E, W, Q) -> Any?) = resolver(FunctionWrapper.Companion.on(function, true))

    fun <E, W, Q, A> resolver(function: suspend (T, E, W, Q, A) -> Any?) = resolver(FunctionWrapper.Companion.on(function, true))

    fun <E, W, Q, A, S> resolver(function: suspend (T, E, W, Q, A, S) -> Any?) = resolver(FunctionWrapper.Companion.on(function, true))

    fun <E, W, Q, A, S, B> resolver(function: suspend (T, E, W, Q, A, S, B) -> Any?) = resolver(FunctionWrapper.Companion.on(function, true))

    fun <E, W, Q, A, S, B, U> resolver(function: suspend (T, E, W, Q, A, S, B, U) -> Any?) = resolver(FunctionWrapper.Companion.on(function, true))

    fun <E, W, Q, A, S, B, U, C> resolver(function: suspend (T, E, W, Q, A, S, B, U, C) -> Any?) = resolver(FunctionWrapper.Companion.on(function, true))
