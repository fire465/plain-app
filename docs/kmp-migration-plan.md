...
- [ ] 6.1 **Notes 页**（7 个 Composable）从 `app/ui/page/notes/` → `shared/commonMain/ui/page/notes/`。等 6.0 完成。注意 `NotePageEffects.kt` 用 `android.app.Activity` + `WindowCompat` + `WindowInsetsCompat`（Android-only），可能需拆出 expect/actual；`NotePage.kt` 用 `LocaleHelper`（Android-only via MainApp）；`NotesPage.kt` 也用 `LocaleHelper`。这 3 个 page 即使 base/components 搬完后仍需 expect/actual 或留 app/。
- [ ] 6.2 **Tags 页**（3 个）→ `shared/commonMain/ui/page/tags/`。等 6.0 完成。`TagsBottomSheet.kt` 用 `DialogHelper.confirmToDelete`（Android-only），可能留 app/。
