...
class SchemaCompilation(
    ...
    private suspend fun introspectionSchemaQuery() = handleOperation(
        QueryDef("__schema", FunctionWrapper.Companion.on<__Schema> { schemaProxy as __Schema })
    )

    private suspend fun introspectionTypeQuery() = handleOperation(
        QueryDef("__type", FunctionWrapper.Companion.on { name: String ->
    ...
    private suspend fun handleObjectType(kClass: KClass<*>) : Type {
        ...
        val __typenameField = handleOperation (
                PropertyDef.Function<Nothing, String?> ("__typename", FunctionWrapper.Companion.on(typenameResolver, true))
        )
    ...
    private suspend fun handleUnionType(union : TypeDef.Union) : Type.Union {
        ...
        val __typenameField = handleOperation (
            PropertyDef.Function<Nothing, String?> ("__typename", FunctionWrapper.Companion.on( { value: Any ->
